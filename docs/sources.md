# Official and Public Sources

Use these when authoring flashcards. **Your official CCAR-F Exam Guide (Partner Academy PDF) is the primary source of truth** for task statements.

## Anthropic / exam

- [Partner Academy — download Exam Guide](https://anthropic-partners.skilljar.com/claude-certified-architect-foundations-certification) (canonical)
- [Direct PDF link](https://everpath-course-content.s3-accelerate.amazonaws.com/instructor%2F6nizmqk8tpzpfjvt6qmmav7rh%2Fpublic%2F1783542750%2FClaude+Certified+Architect+%E2%80%93+Foundations+Exam+Guide.pdf) (may be updated by Anthropic)
- [Exam guide study index in this repo](exam-guide-index.md)
- [Local PDF setup](exam-guide/README.md)
- [About Model Context Protocol (MCP)](https://docs.anthropic.com/en/copilot/concepts/context/mcp)
- [Claude Code overview](https://docs.anthropic.com/en/docs/claude-code/overview)
- [Agent SDK overview](https://docs.anthropic.com/en/docs/agents-and-tools/agent-sdk/overview)
- [Structured outputs](https://docs.anthropic.com/en/docs/build-with-claude/structured-outputs)

## MCP

- [Model Context Protocol introduction](https://modelcontextprotocol.io/introduction)

## Community (unofficial, verify against Exam Guide)

- [Claude Certification Guide — learn / domain crib sheets](https://claudecertificationguide.com/learn) (exam-trap framing; not Anthropic)
- [HarikrishnanKM/claude-architect-exam-guide](https://github.com/HarikrishnanKM/claude-architect-exam-guide)

## Do not use as sources

- Leaked exam questions or exam-taker question banks
- Third-party sites claiming exact pass percentages or unreleased item counts
